...
namespace ScenarioAmender
{
	...
	public class Page_MidGameScenarioEditor : Page_ScenarioEditor
	{
		...
		public Page_MidGameScenarioEditor() : base(Find.Scenario.CopyForEditing())
		{
			...
			this.CheckAllPartsCompatible = (Delegate.CreateDelegate(typeof(Predicate<Scenario>), method) as Predicate<Scenario>);
			this.nextAct = new Action(this.SaveAndClose);
		}
		...
		public override void DoWindowContents(Rect rect)
		{
			...
			base.DoBottomButtons(rect, "Save".Translate(), null, null, true, true);
		}
		...
		public void SaveAndClose()
		{
			...
			foreach (ScenPart scenPart in Current.Game.Scenario.AllParts)
			{
				if (scenPart is ScenPart_PlayerFaction)
				{
					typeof(ScenPart_PlayerFaction).GetField("factionDef", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(scenPart, Find.FactionManager.OfPlayer.def);
				}
				else if ((!(scenPart is ScenPart_GameStartDialog) || Find.GameInitData != null) && !(scenPart is ScenPart_ConfigPage_ConfigureStartingPawns))
				{
		...
		{
			return this.CheckAllPartsCompatible(base.EditingScenario);
		}
	}
}
